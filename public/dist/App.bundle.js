...
function autocomplete(input, latInput, lngInput) {
  ...
  input.on('keydown', function (e) {
    if (e.keyCode === 13) e.prevent.default();
  });
}
...
function typeAhead(search) {
  ...
  searchInput.on('input', function (event) {
    ...
    _axios2.default.get('/api/search?q=' + query).then(function (_ref) {
      var data = _ref.data;

      if (data.length) {
        var html = _dompurify2.default.sanitize(searchResultsHTML(data));

        searchResults.innerHTML = html;
      } else {
        searchResults.innerHTML = '<div class="search__result">No results for ' + query + '</div>';
      }
    }).catch(function (err) {
